...
/// Full-page match organizer — reached from the Manager screen's "Organiser
/// un match" section via its "Créer un match" button.
...
class _MatchFormScreenState extends ConsumerState<MatchFormScreen> {
  ...
  Future<void> _create(ManagerViewModel vm) async {
    await vm.createMatch();
    if (!mounted) return;
    // The form resets to empty only after a fully successful batch create —
    // that's our signal to leave the page; on partial failure stay put so
    // the admin can see the message and retry.
    final form = ref.read(managerViewModelProvider).form;
    if (form.courtId == null && form.slots.isEmpty) {
